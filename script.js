...
function loadRandomPokemon() {
    ...
    // Extract details from imageName (assuming your image names contain this info)
    // Example: '006 Charizard Mega X 1 Fire Dragon'
    ...
    // The name is all the remaining elements
...
function checkAnswer() {
    ...
    if (!isGuessChecked) {
        ...
        //Special case where form name could be easily mixed up with the actual name
        ...
        if (currentPokemon.form){
            currentScore += (nameMatch ? 30 : 0) + (formMatch ? 20 : 0);
        } else {
            currentScore += nameMatch ? 50 : 0;
        }
        ...
        if ((type1Guess === currentPokemon.type2 && type2Guess === currentPokemon.type1) ){
            ...
            currentScore += (type1Match ? 15 : 0) + (type2Match ? 15 : 0);
        } else {
            currentScore += type1Match ? 30 : 0;
        }
        ...
        document.getElementById('pokemon-name').value = ''; // Clear the name textbox
        let selectedGens = getSelectedGenerations();
        document.getElementById('pokemon-gen').selectedIndex = selectedGens[0]-1; // Reset generation dropdown
        document.getElementById('pokemon-type1').selectedIndex = 0; // Reset type1 dropdown
        document.getElementById('pokemon-type2').selectedIndex = 0; // Reset type2 dropdown
        resetInfoCard();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Other setup code...
    // Array of element IDs in the order you want to navigate through
    ...
    nameInput.addEventListener('keydown', function(event) {
        ...
        if (event.key === 'ArrowRight' && !hasTextToRight) {
            // Prevent default behavior and move to the next field
            event.preventDefault();
            document.getElementById('pokemon-gen').focus();
            shouldMoveFocus = false;
        } else if (event.key === 'ArrowLeft' && !hasTextToLeft) {
            // Prevent default behavior and move to the previous field (if there is one)
            event.preventDefault();
    ...
    document.getElementById('pokemon-gen').addEventListener('keydown', function(event) {
        ...
        if (event.key >= '1' && event.key <= '9') {
            ...
            const genIndex = parseInt(event.key, 10) - 1;// Set the selected index of the dropdown
